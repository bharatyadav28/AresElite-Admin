...
export default function TermsAndConditions() {
  ...
  const fetchContents = useCallback(
    async (type) => {
      setIsLoading(true);
      try {
        const res = await axiosInstance.get("/api/admin/privacy_policy", {
          headers: {
        ...
        if (res.data.success) {
          if (res.data.privacyPolicy) {
            // setContent(res.data.privacyPolicy.text);
    ...
    try {
      ...
      if (isDelete) {
        res = await axiosInstance.delete(
          `/api/admin/privacy_policy?id=${content._id}`,
